...
if __name__ == "__main__":
    ...
    for page in range(start_page,stop_page+1):
        print('现在是第',page,'页')
        response = requests.get("https://s.weibo.com/weibo?q=%23%E5%BE%AE%E4%BF%A1%E5%B7%B2%E6%94%AF%E6%8C%81%E6%94%B9%E5%BE%AE%E4%BF%A1%E5%8F%B7%23&from=trendtop_api&refer=index_hot_new&page={}".format(page),headers = headers)
        bs = BeautifulSoup(response.content.decode(errors="ignore"), 'lxml')
        for label in bs.select("#pl_feedlist_index > div:nth-child(1) > div > div > div.card-feed > div.content > p.txt"):
        #内容整理
