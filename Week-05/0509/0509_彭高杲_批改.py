"""
1. 在使用变量之前，应该先考虑变量是否已经被定义了（即使按照逻辑不会运行也尽量严谨）
2. 应优先代码可以正常运行，没有错误（变量名不同造成的错误）
"""
...
if __name__ == '__main__':
    ...
    response = requests.get(
        "https://weibo.com/p/1005051260555362/home?is_search=0&visible=0&is_ori=1&is_tag=0&profile_ftype=1&page=1#feedtop",
        headers=headers)
    ...
    for i in range(total_page):
        ...
        response_page = requests.get(
            "https://weibo.com/p/1005051260555362/home?is_search=0&visible=0&is_ori=1&is_tag=0&profile_ftype=1&page=" + str(
                p) + "#feedtop",
            headers=headers)
        ...
        for q in page.select("#Pl_Official_MyProfileFeed__19 > div > div"):
            ...
            if q["action-data"]:
                ...
                weibo_time = q.select_one(
                    "# div > div.WB_feed_detail.clearfix > div.WB_detail > div.WB_from.S_txt2 > a:nth-child(1)").text
                weibo_forward = q.select_one(
                    "# div > div.WB_feed_handle > div > ul > li:nth-child(2) > a > span > span > span > em:nth-child(2)").text
                weibo_comment = q.select_one(
                    "# div > div.WB_feed_handle > div > ul > li:nth-child(3) > a > span > span > span > em:nth-child(2)").text
                weibo_like = q.select_one(
