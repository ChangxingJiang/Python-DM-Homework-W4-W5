...
for index,div, in enumerate(divs):#遍历所有问题
    question = div.a.string
    questions.append(question)#将问题添加进列表
    href = div.a['href']
    id1 = href.split('/')[-3]
    id2 = href.split('/')[-1]
    newurl = 'https://www.zhihu.com/question/%s/answer/%s'%(id1,id2)#找到每个问题的链接
    response = requests.get(newurl,headers=headers)
    onehtml = response.text
    onesoup = BeautifulSoup(onehtml,'html.parser')#解析新网页内容
    for follower_number in onesoup.find('strong',{'class':'NumberBoard-itemValue'}):
        follower_numbers.append(follower_number.string)#添加关注者数列表
    for view_number in onesoup.find('a',{'class':'QuestionMainAction ViewAll-QuestionMainAction'}):
...
with open(r'E:\python学习\test\zhihuOUT.txt','w', encoding='utf-8',errors='ignore',newline='') as f:#打开文件
    for item in list1:
